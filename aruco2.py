...
while exitt:
    ...
    for a in range(len(corners)):
        for b in range(a+1, len(corners)):
            if abs(int(corners[a][0][1][0]) - int(corners[b][0][1][0])) < 30:
                if a not in indexs:
                    indexs.append(a)
         #       print(int(corners[a][0][1][0]) , int(corners[b][0][1][0]), indexs)
    #print(indexs)
    if len(indexs) != 0:
    ...
    if len(corners) > 1:
        ...
        for i in range(2):
            ...
            arucos.append([cX, cY, dikler[biggest_index]])
            dikler.pop(biggest_index)
            corners.pop(biggest_index)
        #print(arucos)
...
#BU KOD TEK ARUCO ICIN OLSUN CIFT ARUCO ICIN BUNU KOPYALAYIP BIR TANE DAHA YAP BENCE
        ...
        # ortalama kismi
        mid = [(arucos[0][0]+arucos[1][0])//2, (arucos[0][1]+arucos[1][1])//2, (arucos[0][2]+arucos[1][2])//2]
    ...
    #center yukseklık 75 cm        uzaklık    110 cm ıken center        kamera 50cm yukarda            logide piksel (426, 121)    kosegen 250
        ortalandi = 0
        if mid[0] > istenilen_koordinat_x+hata_payi_x:
            print("Right")
            drive(0, -ANGULARSPEED, 0)
            son_hareket = 1
        elif mid[0] < istenilen_koordinat_x-hata_payi_x:
            drive(0, ANGULARSPEED, 0)
            print("Left")
            son_hareket = 2
        else:
        ...
        if ortalandi == 1:            #uzaklik
            if mid[2] < istenilen_dik:
                print("Forward")
                son_hareket = 0
                drive(LINEARSPEED, 0, 0)
            else:
                print("5 meters forward")
                drive(LINEARSPEED, 0, 0)
                time.sleep(2)
                print("Reaching")
                for i in range(10):
